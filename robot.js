...
export function makeMetalRobot() {
  ...
  const bodyL 
    = new THREE.Mesh(new THREE.CylinderGeometry(bodyD/2, bodyD/2, bodyH, seg, 1,false,0,Math.PI),metalMaterial);
...
eyeL.position.set(eyeSep/2 ,eyePos+gap, headD/2+gap);
head.add(eyeL);
const eyeR = new THREE.Mesh(circleGeometry, blackMaterial);
eyeR.position.set(-eyeSep/2 ,eyePos+gap, headD/2+gap);
head.add(eyeR);
//口の設定
const triangleGeometry = new THREE.BufferGeometry();
    const triangleVertices = new Float32Array([
